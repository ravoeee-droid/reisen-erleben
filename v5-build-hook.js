...
if(actual!==expected) throw new Error(`STOP-SHIP: V5 source checksum mismatch`);
...
buildSync({stdin:{contents:src['v5-entry.js'],resolveDir:root,sourcefile:'v5-entry.js',loader:'js'},outfile:path.join(assets,'v5.js'),bundle:true,minify:true,target:['es2020'],format:'iife',legalComments:'eof',sourcemap:false});
...
for(const file of htmlFiles){let s=fs.readFileSync(file,'utf8');s=s.replace(/<script[^>]+src=["']\/assets\/(?:v4|tobi|v5)\.js["'][^>]*><\/script>/gi,'').replace(/<link[^>]+href=["']\/assets\/v5\.css["'][^>]*>/gi,'');s=s.replace(/https:\/\/(?:www\.)?reisenunderleben\.net\/images\/[^"'\s)<>]+/gi,m=>proxyImage(m));let heroDone=false;s=s.replace(/<img\b([^>]*?)>/gi,(tag)=>{if(/class=["'][^"']*brand/i.test(tag)||/logo/i.test(tag))return tag;if(!heroDone&&/hero/i.test(s.slice(Math.max(0,s.indexOf(tag)-500),s.indexOf(tag)))){heroDone=true;let t=tag.replace(/\sloading=["'][^"']*["']/i,'').replace(/\sfetchpriority=["'][^"']*["']/i,'');return t.replace(/>$/,' loading="eager" fetchpriority="high" decoding="async">');}if(!/\sloading=/.test(tag))return tag.replace(/>$/,' loading="lazy" decoding="async">');return tag;});s=s.replace('</head>','<link rel="stylesheet" href="/assets/v5.css"></head>').replace('</body>','<script defer src="/assets/v5.js"></script></body>');fs.writeFileSync(file,s,'utf8');}
...
if(!fs.existsSync(path.join(assets,'v5.js'))||fs.statSync(path.join(assets,'v5.js')).size<5000)throw new Error('STOP-SHIP: V5 bundle invalid');
console.log(`R&E V5 applied: ${htmlFiles.length} HTML pages, V4 JS removed, images proxied, release gates passed`);
